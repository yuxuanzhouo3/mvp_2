...
ADD CONSTRAINT recommendation_cache_category_preference_hash_key UNIQUE (category, preference_hash);
...
CREATE POLICY "Users can view own recommendation history" ON recommendation_history FOR
SELECT USING (auth.uid () = user_id);

DROP POLICY IF EXISTS "Users can insert own recommendation history" ON recommendation_history;

CREATE POLICY "Users can insert own recommendation history" ON recommendation_history FOR
INSERT
WITH
...
CREATE POLICY "Users can update own recommendation history" ON recommendation_history FOR
...
CREATE POLICY "Users can view own clicks" ON recommendation_clicks FOR
SELECT USING (auth.uid () = user_id);

DROP POLICY IF EXISTS "Users can insert own clicks" ON recommendation_clicks;

CREATE POLICY "Users can insert own clicks" ON recommendation_clicks FOR
INSERT
WITH
    CHECK (auth.uid () = user_id);
...
CREATE POLICY "Anyone can read cache" ON recommendation_cache FOR
SELECT USING (true);
...
CREATE POLICY "Service role full access to recommendation_history" ON recommendation_history FOR ALL USING (auth.role () = 'service_role');
...
CREATE POLICY "Service role full access to recommendation_clicks" ON recommendation_clicks FOR ALL USING (auth.role () = 'service_role');

DROP POLICY IF EXISTS "Service role full access to recommendation_cache" ON recommendation_cache;

CREATE POLICY "Service role full access to recommendation_cache" ON recommendation_cache FOR ALL USING (auth.role () = 'service_role');
